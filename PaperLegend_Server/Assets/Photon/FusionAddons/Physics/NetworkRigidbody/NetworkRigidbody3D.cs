...
namespace Fusion.Addons.Physics {
  ...
  public class NetworkRigidbody3D : NetworkRigidbody<Rigidbody, RunnerSimulatePhysics3D> {
    ...
    protected override void CaptureRBPositionRotation(Rigidbody rb, ref NetworkRBData data, bool useWorldSpace) {
      ...
      rot = IsValidRotation(rot) ? NormalizeRotation(rot) : Quaternion.identity;
    ...
    protected override void ApplyRBPositionRotation(Rigidbody rb, Vector3 pos, Quaternion rot) {
      if (!IsValidRotation(rot)) {
        rot = rb != null && IsValidRotation(rb.rotation) ? rb.rotation : Quaternion.identity;
      } else {
    ...
    private static bool IsValidRotation(Quaternion rot) {
      float sqrMagnitude = rot.x * rot.x + rot.y * rot.y + rot.z * rot.z + rot.w * rot.w;
      return !float.IsNaN(sqrMagnitude) &&
             !float.IsInfinity(sqrMagnitude) &&
             sqrMagnitude > 0.0001f;
    }

    private static Quaternion NormalizeRotation(Quaternion rot) {
      float magnitude = Mathf.Sqrt(rot.x * rot.x + rot.y * rot.y + rot.z * rot.z + rot.w * rot.w);
      return magnitude > 0.0001f
        ? new Quaternion(rot.x / magnitude, rot.y / magnitude, rot.z / magnitude, rot.w / magnitude)
        : Quaternion.identity;
    }
